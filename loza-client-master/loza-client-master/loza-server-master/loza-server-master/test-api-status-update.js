...
async function testApiStatusUpdate() {
  try {
    console.log('Testing API order status update...');
    
    // First, let's get an existing order to test with
    const ordersResponse = await axios.get('http://localhost:5000/api/orders/get-orders', {
    ...
    const updateResponse = await axios.put(
      `http://localhost:5000/api/orders/update-order-status/${testOrder._id}`,
      { status: newStatus },
      { withCredentials: true }
    );
    ...
    // Verify the update
    const verifyResponse = await axios.get(`http://localhost:5000/api/orders/get-order/${testOrder._id}`, {
      withCredentials: true
    });
